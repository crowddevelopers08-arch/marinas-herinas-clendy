...
const SYMPTOM_VIDEO =
  "https://ik.imagekit.io/tpucbav8z/hernia-symptom-video-iphone.mp4";
...
export function HeroSection() {
  ...
  // Set webkit-playsinline + muted imperatively for iOS Safari
  useEffect(() => {
  ...
  // Keep DOM muted property in sync with React state (React bug: doesn't sync the property)
  useEffect(() => {
    const video = heroVideoRef.current;
    if (!video) return;
    video.muted = muted;
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={`${wrap} grid grid-cols-2 items-center gap-14 max-[900px]:grid-cols-1 max-[900px]:gap-5`}
        >
          {/* Left Content */}
          <div className="flex flex-col items-start max-[900px]:contents">
            ...
            <div className="flex flex-col items-start max-[900px]:order-3 max-[900px]:w-full max-[900px]:pt-3">
              ...
              <div className="mb-6 flex flex-wrap items-center gap-3 max-[620px]:w-full max-[620px]:flex-col max-[620px]:gap-2.5">
                <a
                  href="#book"
                  className={`${button} max-[620px]:w-full max-[620px]:justify-center`}
                >
